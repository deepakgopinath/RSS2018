...
% end
%% Success rate
success_rate_matrix = zeros(length(condList), length(spaceList)); %columns represent space. Rows represent 
for i=1:length(spaceList)
    data = before_timeout_trials(:,:, i);
    success_rate_matrix(:, i) = mean(data)';
end
fprintf('Average success rate across different spaces POT, ENT, KL, DISAMB respectively\n');
disp(mean(success_rate_matrix, 2)); %ENT HAS THE LOWEST. KL IS DOING GOOD OVERALL. 
...
%         pause;
    ...
    xt = get(gca, 'XTick');
    yt = get(gca, 'YTick');
%     axis([xlim    0  ceil(max(yt)*1.1)]); hold on;
     axis([xlim    0  1.3]); hold on;
%     plot(xt([1 3]), [1 1]*max(yt)*1.1, '-k',  [0.9, 1,1.1]*mean(xt([1 3])), [1,1,1]*max(yt)*1.15, '*k')
    for j=1:length(pvalues)
        ...
        elseif pvalues(j) < 0.05
            xtl = c(j, 1); xth = c(j, 2); yval = max(maxx(xtl), maxx(xth));
            plot([xtl, xth], [1 1]*yval*(1.02 + j*0.04), '-k', [mean([xtl, xth]) - 0.035, mean([xtl, xth]) + 0.035], [1,1]*yval*(1.04+ j*0.04), '*k');
...
%         col = data(before_timeout_trials(:,j,i) == 1, j);
    ...
    %Almost normal data. Requires not kruskal wallis but something else. 
    [p,tbl,stats] = anova1(x, cond, 'off');
    c = multcompare(stats, 'Display', 'off');
    sps{i} = subplot(2,2,i);
    bh1 = boxplot(x, cond, 'whisker', 1.5); 
    set(bh1(:,1),'linewidth',2); set(bh1(:,2),'linewidth',2); set(bh1(:,3),'linewidth',2); set(bh1(:,4),'linewidth',2);
    title(sps{i}, strcat('\bf \fontsize{15}',title_list{i})); 
    grid on; xlabel('\bf Mode Switch Assistance Type'); ylabel('\bf \fontsize{8} Trajectory length (m)');
    pvalues = c(:, 6);
    %significance plotting
    xt = get(sps{i}, 'XTick');
    yt = get(sps{i}, 'YTick');
...
%         elseif pvalues(j) < 0.05
%             xtl = c(j, 1); xth = c(j, 2); yval = max(maxx(xtl), maxx(xth));
%             plot([xtl, xth], [1 1]*yval*(1.05 + j*0.1), '-k', [mean([xtl, xth]) - 0.035, mean([xtl, xth]) + 0.035], [1,1]*yval*(1.1+ j*0.1), '*k');
%         elseif pvalues(j) < 0.01
%             xtl = c(j, 1); xth = c(j, 2); yval = max(maxx(xtl), maxx(xth));
%             plot([xtl, xth], [1 1]*yval*(1.05 + j*0.1), '-k', mean([xtl, xth]), yval*(1.1 + j*0.1), '*k');
%         end
%     end
    for j=1:length(pvalues)
        if pvalues(j) < 0.001   
            xtl = c(j, 1); xth = c(j, 2); yval = max(maxx(xtl), maxx(xth));
            plot([xtl, xth], [1 1]*yval*(1.02 + j*0.04), '-k', [mean([xtl, xth]) - 0.1, mean([xtl, xth]), mean([xtl, xth]) + 0.1], [1,1,1]*yval*(1.04 + j*0.04), '*k');
        elseif pvalues(j) < 0.05
            xtl = c(j, 1); xth = c(j, 2); yval = max(maxx(xtl), maxx(xth));
            plot([xtl, xth], [1 1]*yval*(1.02 + j*0.04), '-k', [mean([xtl, xth]) - 0.035, mean([xtl, xth]) + 0.035], [1,1]*yval*(1.04+ j*0.04), '*k');
        elseif pvalues(j) < 0.01

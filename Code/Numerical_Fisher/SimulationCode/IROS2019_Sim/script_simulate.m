...
for ii=1:N
   ...
   filename = strcat('R22_DATA/RUN_2_DATAR22_', num2str(ii), '.mat');
...
for ii=1:N
   ...
   filename = strcat('R33_DATA/RUN_2_DATAR33_', num2str(ii), '.mat');
   save(filename, 'ng', 'nd', 'cm', 'num_modes', 'xg', 'xr_true', 'intent_type','random_goal_index');
   for jj=1:length(vars)
        for kk=1:length(types)
            save(filename, strcat(vars{jj}, types{kk}), '-append');
...
for ii=1:N
   ...
   filename = strcat('SE3_DATA/RUN_2_DATASE3_', num2str(ii), '.mat');

...
title('ENT GOAL MATCH'); xlabel('Time Steps'); ylabel('Matched/Unmatched'); ylim([0.0, 1.0]);
subplot(4,2,2);
scatter(1:length(curr_goal_ENT), curr_goal_ENT, 'r', 'filled'); grid on;
title('ENT GOAL INFERENCE'); xlabel('Time Steps'); ylabel('Goal Index');ylim([1, ng]);
...
title('DISAMB GOAL MATCH'); xlabel('Time Steps'); ylabel('Matched/Unmatched');ylim([0.0, 1.0]);
subplot(4,2,6);
scatter(1:length(curr_goal_DISAMB), curr_goal_DISAMB,'g', 'filled'); grid on;
title('DISAMB GOAL INFERENCE'); xlabel('Time Steps'); ylabel('Goal Index');ylim([1, ng]);

subplot(4,2,7);
scatter(1:length(curr_goal_POT), curr_goal_POT == random_goal_index, 'k','filled'); grid on;
title('POT GOAL MATCH'); xlabel('Time Steps'); ylabel('Matched/Unmatched');ylim([0.0, 1.0]);
subplot(4,2,8);
scatter(1:length(curr_goal_POT), curr_goal_POT, 'k', 'filled'); grid on;
title('POT GOAL INFERENCE'); xlabel('Time Steps'); ylabel('Goal Index');ylim([1, ng]);
...
num_mode_switches = zeros(total_subjects, length(condList), length(spaceList));
%%
for i=1:length(spaceList)
    dirname = strcat('NEW_',spaceList{i}, '_DATA');
    fnames = dir(dirname);
    ...
    for index=3:3 + (total_subjects-1)
        ...
        if  any((curr_goal_KL == random_goal_index).*alpha_KL)
            initial_alpha(k, 3, i) = find((curr_goal_KL == random_goal_index).*alpha_KL > 0.0, 1)*delta_t;
        end

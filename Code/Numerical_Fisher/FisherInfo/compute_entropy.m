function [ Upsilon ] = compute_entropy(uh, varargin)
...
if strcmp(intent_type, 'conf')
    pg = compute_conf(uh, curr_x); %confidence function does not care of what the current pg is and therefore ignores pg0
elseif strcmp(intent_type, 'dft')
    ...
    for i=1:length(t_stamps)-1
        pgs_project(:, i+1) = compute_p_of_g_dft(uh, curr_x, pgs_project(:, i));
        curr_x = sim_dyn(curr_x, uh);
    end
...
elseif strcmp(intent_type, 'bayes')
    ...
    for i=1:length(t_stamps)-1
        pgs_project(:, i+1) = compute_bayes(uh, curr_x, pgs_project(:, i));
        curr_x = sim_dyn(curr_x, uh);
    end
    pg = pgs_project(:, end);
end
